...
This project is for easy php intergration with the ebits wallet daemon.
...
// Make calls to Ebitsd as methods for your object. Responses are returned as an array.
// Examples:
$Ebits->getblockcount();
...
class Ebits
{
     ...
     * @param string $url
     */
    public function __construct($username, $password, $host = 'localhost', $port = 9901, $url = null)
    {
    ...
    public function __call($method, $params)
    {
        ...
        if ($this->response['error']) {
            // If Ebitsd returned an error, put that in $this->error
            $this->error = $this->response['error']['message'];
        } elseif ($this->status != 200) {
            // If Ebitsd didn't return a nice error message, we need to make our own
            switch ($this->status) {
                case 400:
